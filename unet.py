...
class UNet(nn.Module):
    def __init__(self, n_channels=4, n_classes=2, bilinear=True, base_filters=32):
        super().__init__()
...
# -----------------------------
# Dice loss + BCE combination
# -----------------------------
def dice_loss(pred, target, smooth=1.0, weight=None):
    ...
    if weight:
        weights = torch.where(target > 0.5, weight_pore, 1.0)
        loss = loss * weights.mean(dim=(1,2,3))
    
    return loss.mean()

def combined_loss(pred, target, weights=None):
    bce = F.binary_cross_entropy_with_logits(pred, target)
    d_loss = dice_loss(pred, target, weights)
    return bce + d_loss